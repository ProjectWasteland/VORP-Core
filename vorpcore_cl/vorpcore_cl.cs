        ...
        public vorpcore_cl()
        {
            EventHandlers["getCore"] += new Action<CallbackDelegate>(cb =>
            {
                var corefunctions = new Dictionary<string, dynamic>
                {
                        ...
                        {
                            Exports["vorp_core"]
                                    .DisplayRightTip(text, miliseconds);
                        }),
                        ["displayObjetive"] = new Action<string, int>((text, miliseconds) =>
                        {
                            Exports["vorp_core"]
                                    .DisplayObjetive(text, miliseconds);
                        }),
                        ["displayTopCenterNotification"] =
                                new Action<string, string, int>((text, location, miliseconds) =>
                                {
                                    Exports["vorp_core"]
                                            .DisplayTopCenterNotification(text,
                                                                          location, miliseconds);
                                }),
                        ["displayLeftNotification"] =
                                new Action<string, string, string, string, int>(async (title, text, dic, icon,
                                                                                    miliseconds) =>
                                                                                {
                                                                                    await Miscellanea
                                                                                            .LoadTexture(dic);
                                                                                    Exports["vorp_core"]
                                                                                            .DisplayLeftNotification(title,
                                                                                                text, dic,
                                                                                                icon,
                                                                                                miliseconds);
                                                                                })
